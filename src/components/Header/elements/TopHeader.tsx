...
const TopHeader: FC = () => {
    ...
    const setLanguage = useCallback(async (event: MouseEvent<HTMLButtonElement>) => {
        const language = event.currentTarget.dataset.language ?? LANGUAGES.VI

        const {data} = await axios.post<{ language: string, reload: boolean }>(
            "/api/language",
            {
                language: language
            }
        ...
        <Container className="bg-primary flex justify-between items-center">
            ...
            <Root>
                <Trigger asChild={true}>
                    <button
                        className="outline-none min-w-28 text-zinc-200 flex items-center  space-x-1"
                    >
                        ...
                        {
                            language === LANGUAGES.VI ? "Tiếng Việt" : "English"
                        }
                    </span>
                ...
                >
                    <Close
                        ...
                        data-link-active={language === LANGUAGES.VI}
                        onClick={setLanguage}
                    ...
                    <Close
                        ...
                        data-link-active={language === LANGUAGES.EN}
                        onClick={setLanguage}
                        className="text-left block w-full outline-none cursor-pointer px-2 data-link-active:text-white data-link-active:bg-primary"
                    >
                        English
